...
| Publisher account | [skundu42](https://github.com/skundu42) |
| Website | https://github.com/skundu42/web-mcp-skill |
| Support URL | https://github.com/skundu42/web-mcp-skill/blob/main/SUPPORT.md |
...
## Descriptions and starter prompts

The descriptions and three starter prompts are preserved in [plugin.json](plugin.json), under `interface`. Its policy URLs and logo paths are included in the release package. The support URL and availability are portal fields; they are recorded here rather than added as unsupported manifest keys.
...
Requirements are based on the [official submission guide](https://developers.openai.com/plugins/deploy/submission), checked August 31, 2026. Do not treat local package validation as submission approval.

## Logo provenance

Created using the built-in imagegen tool. The design is a browser outline connected to three mint-teal nodes on a navy background. An initial version with transparency artifacts was discarded; the shipped PNG is fully opaque. No trademark clearance is claimed.

<details>
<summary>Final image-edit prompt</summary>

Edit this logo. Preserve the browser outline and three-branch connector design, placement, and overall scale. FIX the defective background and edges: the entire image must be fully OPAQUE, with a perfectly solid midnight navy #0B1525 background filling every pixel outside the mark, including the entire interior of the browser outline. Absolutely NO transparent pixels, no alpha cutouts, no checkerboard, no black speckling, no white noise, no texture or distressed effect. Redraw all edges smooth and crisp. Browser frame, header bar, and inside connecting node are solid off-white. Three-branch connector is solid mint teal. Flat vector-like logo, no shadows or shading. Only the clean single logo, square canvas, no text. Save a fully opaque PNG.
